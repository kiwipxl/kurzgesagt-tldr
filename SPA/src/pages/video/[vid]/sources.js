...
const Component = (props) => {
  ...
  for (const keyPoint of props.sources.keyPoints) {
    ...
    for (const content of keyPoint.content) {
      switch (content.type) {
        ...
        case 'citation':
          ...
          renderKeyPointContent.push(
            <span className="sources-quote">{content.data.quote}</span>
          );
    ...
    renderContent.push(
      <div key={index} className="sources-key-point">
        {renderKeyPointContent}
      </div>
    );
      ...
      <Card className="video-details-card">
        <Card.Body>
          <p className="info-box">
            This section is the citations and further reading provided by
            Kurzgesagt at
            <a href={props.sources.url} target="_blank">
              {' ' + props.sources.url}
            </a>
          </p>
